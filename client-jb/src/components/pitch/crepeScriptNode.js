...
export async function makeCrepeScriptNode(
  audioContext,
  bufferSize,
  pitchCallback,
  pitchVectorCallback
  ...
  async function initTF() {
    try {
      ...
      console.log(`now call loadModel`)
      //window.model = await tf.loadModel('model/model.json');
      //await tf.loadModel('/model/model.json'); // in /public folder
