...
const def_segments = [1, .8, .5];

const circleScale=.82;
//=====================================================================================================

const makePieStructure = function (svg, center={x:0, y:0}, radius=100, labels=def_labels,  m_width=200, m_height=200, segments) {
    for (let i=0;i<3;i++) {
    ...
    for (let i = 0; i < numRadialLines; i++) {
      ...
      line.setAttributeNS(null,"x2", center.x + radius * Math.cos((i * 2 * Math.PI) / numRadialLines));
      line.setAttributeNS(null,"y2",center.y + radius * Math.sin((i * 2 * Math.PI) / numRadialLines));
...
//=====================================================================================================
const PieChart = ({labels=def_labels, radius=100, m_width=200, m_height=200, segments=def_segments}) => {
  ...
  useEffect(() => {
    ...
    makePieStructure(svgRef.current, pie.center, pie.radius, labels,  m_width, m_height, segments);

    let i=0;
    segments.forEach(segmentr => {
      const arc = document.createElementNS(static_xmlns, 'path');
      arc.setAttribute('d', circleSegmentPath(pie.center, segmentr*pie.radius*circleScale, (i * 2 * Math.PI) / pie.numSegments, ((i+1) * 2 * Math.PI) / pie.numSegments));
