import React from 'react';
import ControlBarPanel from './ControlBarPanel';

/**
 * The ControlBarShortcutsPanel component displays a panel with keyboard shortcuts.
 *
 * Props:
 * - showInfo (boolean): Determines whether to show the shortcuts panel.
 * - playbackMode (boolean): Indicates if playback mode is active.
 * - practiceModeOn (boolean): Indicates if practice mode is active.
 * - isMac (boolean): Indicates if the user is on a Mac.
 *
 * The component:
 * - Displays different shortcuts based on the playbackMode and practiceModeOn props.
 * - Shows the appropriate key combinations for Mac and non-Mac users.
 *
 * The shortcuts include:
 * - Reset
 * - Toggle Stats / Switch Modes
 * - Toggle Listen
 * - Toggle Playback / Toggle Practice
 * - Toggle Record
 * - View Shortcuts
 */
const ControlBarShortcutsPanel = ({
  showInfo,
  playbackMode,
  practiceModeOn,
  isMac,
}) => {
  return (
    <ControlBarPanel show={showInfo}>
      <div className={`${practiceModeOn ? 'pb-2' : 'pb-8'}`}>
        <p className="font-bold text-white text-2xl mb-3">Shortcuts</p>
        <div className="flex justify-between space-x-4">
          <div>
            {(playbackMode || practiceModeOn) && (
              <div>
                <span className="text-white font-bold">
                  {isMac ? 'Command' : 'Ctrl'} + Shift + R:{' '}
                </span>
                <span className="text-white opacity-75">Reset</span>
              </div>
            )}
            <div>
              <span className="text-white font-bold">
                {playbackMode ? `S: ` : `M: `}
              </span>
              <span className="text-white opacity-75">
                {playbackMode ? 'Toggle Stats' : 'Switch Modes'}
              </span>
            </div>
          </div>
          <div>
            {!playbackMode && practiceModeOn && (
              <div>
                <span className="text-white font-bold">L: </span>
                <span className="text-white opacity-75">Toggle Listen</span>
              </div>
            )}
            {practiceModeOn && (
              <div>
                <span className="text-white font-bold">P: </span>
                <span className="text-white opacity-75">
                  {playbackMode ? 'Toggle Playback' : 'Toggle Practice'}
                </span>
              </div>
            )}
            {!playbackMode && !practiceModeOn && (
              <div>
                <span className="text-white font-bold">R: </span>
                <span className="text-white opacity-75">Toggle Record</span>
              </div>
            )}
          </div>
          <div>
            <div>
              <span className="text-white font-bold">I: </span>
              <span className="text-white opacity-75">View Shortcuts</span>
            </div>
          </div>
        </div>
      </div>
    </ControlBarPanel>
  );
};

export default ControlBarShortcutsPanel;
