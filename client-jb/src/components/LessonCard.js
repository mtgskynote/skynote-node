...
const LessonCard = ({ title, skill, level, stars, xml, id }) => {
  ...
  return (
    <Card
      className={`h-52 w-96 transition ease-in-out delay-100 max-w-sm relative rounded-sm overflow-hidden shadow-md hover:shadow-lg ${bgColor[0]} ${bgColor[1]} hover:cursor-pointer`}
      onClick={handleClick}
      id={id}
    >
      <CardContent>
